...
- **Errors**. Laravel defaults: 401 `{ "message": "Unauthenticated." }`, 403, 404, and 422 `{ "message", "errors": { field: [...] } }`. Business-rule violations additionally carry a stable `code` (e.g. `attachment_requires_pro`). Abuse rate limits return 429 with a `Retry-After` header and their own `code`. The catalog lives in [../validation.md](../validation.md); frontends key off codes, never message strings.
