...
- **One bundle per domain**, not one shared bundle: one `entry.ts` per domain under `resources/ts/domains/{app,admin,public}/`, each declaring its own `pages/` directory. Pages are `domains/{domain}/pages/{resource}/{Action}.page.svelte`, so `Inertia::render('jobs/Index')` resolves to `pages/jobs/Index.page.svelte`. The `.page.svelte` extension narrows both the Vite glob and `inertia.pages.extensions`, so a plain `.svelte` file in that tree is a component and never a page. The apex must never ship admin code, and a marketing visitor must not download the product UI.
...
- Components: headless primitives (Bits UI, pinned) styled by the design system tokens, in `resources/ts/components/{group}/{name}/` with groups mirroring the design spec sections (actions, forms, data, feedback, navigation, layout). One folder per component, each with an `index.ts`, reached through the `$components` alias; there is no library-wide barrel, which would make a single import pull every component and undo the per-domain bundle split. Components are built by the phase whose screens need them, not up front. Specs and the mirror live in `design/` at the repo root. The data table is built in-house (no TanStack Table).
