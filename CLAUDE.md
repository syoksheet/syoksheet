...
- **The code is the design system's source of truth.** `resources/scss/` holds the tokens and `resources/ts/components/` the components; when anything disagrees with them, the code wins. `design/docs/` holds component specs (behavior, a11y) and is a reference, not an authority, and it is slated for replacement by Figma. The Claude Design projects are a wireframing and visualization surface: quick to sketch in, deliberately not optimized, and expected to drift. Never chase that drift, never sync code back to them as an obligation, and never treat a value read from them as authoritative. Borrow from them deliberately or not at all.
