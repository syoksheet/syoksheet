...
| 2c | Marketing shell | The apex pages that depend on nothing: homepage, `/for-organizations`, `/pricing`, and `security.txt` at `/.well-known/`. Inertia with SSR on the apex. Brings the components those pages need (Button, Badge, Tag, Avatar) under the build-as-needed rule. No GTM, no cookie banner and no GA4: those need a cookie policy and a privacy policy, both deferred, and Cloudflare Analytics covers traffic without consent. Pulled forward from Phase 14, which keeps the walls, the directory and view tracking because those need brags and organizations to exist | syoksheet-docs → marketing/pages.md, product/pricing.md |
...
| 14 | Public pages & analytics | User walls, org walls + pins, org directory (from the marketing designs; missing pages designed first), view tracking + analytics tables + dashboards, custom wall URL, PDF export | features/public/, features/users/pdf-export.md |
...
- Phase 2c ships before auth so the marketing site can start earning search traffic while the product is built. It needs no database, no login and no staging.
